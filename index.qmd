...
**Due date: October 4th at 11:59pm**
...
#Test using sample_mean
sample_mean(c(2,6,3,4,5,9,1,5,2,4,8))
# Confirmation using mean
mean(c(2,6,3,4,5,9,1,5,2,4,8))
# Test using sample_mean
sample_mean(c(324,334,354,345,341,322,347,362,342))
# Confirmation using mean
mean(c(324,334,354,345,341,322,347,362,342))

sample_sd <- function(inp) {
    smpl_m <- sample_mean(inp)
    big_n <- length(inp)
    
    st_dev <- sqrt(sum((inp - smpl_m)^2)/(big_n - 1))
    return(st_dev)
}

#Test using sample_sd
sample_sd(c(2,6,3,4,5,9,1,5,2,4,8))

#Confirmation using sd
sd(c(2,6,3,4,5,9,1,5,2,4,8))

#Test using sample_sd
sample_sd(c(324,334,354,345,341,322,347,362,342))

#Confirmation using sd
sd(c(324,334,354,345,341,322,347,362,342))
```
...
calculate_CI <- function(x, conf = 0.95) {
    ...
    t_score <- qt(p = alpha / 2, df = degrees_freedom, lower.tail = FALSE)
    adjusted_tscore <- t_score * (sample_sd(x)/sqrt(length(x)))
    lower <- sample_mean(x) - adjusted_tscore
    upper <- sample_mean(x) + adjusted_tscore
    return(c(lower, upper)) 
}

# Test 
fit_test <- c(2,6,3,4,5,9,1,5,2,4,8,7,2,3,6,5,8,1,2,5,3,7,8,4,5,2,3,7,4,9,8,2,4,5,3,5,1,2,7,6,3,4,5,8,6,8,7,4,3,4,9,4,5,6,3,5,2,4)
calculate_CI(fit_test, conf = 0.95)

# Confirmation test
#try_attempt <- t.test(fit_test)
#try_attempt$conf.int[1] # lower bound
#try_attempt$conf.int[2] # upper bound
test_attempt <- lm(fit_test ~ 1)
confint(test_attempt, level = 0.95)
...
library(ggplot2)
library(dplyr)
# Want to merge across by year and identify min and max for temp
simpl_temp <- obj %>%
    filter(year >= 2014) %>%
...
ggplot(temp_long, aes(x = year, y = temperature, color = min_max))+
geom_point()+
geom_line()+
